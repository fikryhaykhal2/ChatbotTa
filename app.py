...
import zipfile # Pastikan ini di-import
...
def download_assets():
    """Mengunduh model ZIP dan mengekstraknya di lingkungan cloud."""
    st.info("Mengunduh dan mengekstrak aset dari Google Drive...")
    ...
    try:
        ...
        if not os.path.isdir("models/sbert_model"):
            ...
            with zipfile.ZipFile(sbert_zip_path, 'r') as zip_ref:
                # Penting: Ekstraksi ke folder 'models/' agar menghasilkan 'models/sbert_model'
                zip_ref.extractall("models/")
            os.remove(sbert_zip_path) 
            st.success("Ekstraksi Model SBERT Selesai.")
        ...
        if not os.path.isdir("models/title_generator"):
            ...
            with zipfile.ZipFile(generator_zip_path, 'r') as zip_ref:
                zip_ref.extractall("models/")
            os.remove(generator_zip_path) 
            st.success("Ekstraksi Model Generator Selesai.")
            
        st.success("Pengunduhan dan Ekstraksi Aset Final Selesai.")
